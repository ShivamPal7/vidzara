    ...
    return (
      <button
        ...
        className={cn(
          ...
          // Mobile: icon-only circle or pill with label
          showLabelOnMobile
            ? "h-9 px-3.5 gap-1.5 w-auto"
            : "size-9 p-0 gap-0",
          // sm+: pill with label
      ...
      >
        ...
        <span className={cn(showLabelOnMobile ? "inline" : "hidden sm:inline", "whitespace-nowrap")}>{label}</span>
        {showChevron && (
          <ChevronDown className={cn(showLabelOnMobile ? "block" : "hidden sm:block", "size-3 shrink-0 opacity-50")} />
        )}
      </button>
    )
  }
