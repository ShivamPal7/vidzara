...
interface TrafficSourcesProps {
  data: Array<[string, number, number]>; // sourceType, views, watchMinutes
}
...
export function TrafficSources({ data }: TrafficSourcesProps) {
  ...
  const chartData = data.map(([sourceType, views, watchMinutes]) => ({
    name: sourceMapping[sourceType] || "Other",
    views,
  ...
  return (
    <Card className="rounded-xl border border-border/50 bg-card/30 p-5 flex flex-col min-h-[400px] h-auto lg:h-[400px] w-full min-w-0 overflow-hidden">
      <div className="flex items-center gap-2 mb-6">
        <IconTrafficLights className="w-5 h-5 text-primary" />
      ...
      <div className="flex-1 flex flex-col sm:flex-row items-center gap-6 sm:gap-4 min-h-0 w-full">
        ...
        <div className="w-full sm:w-1/2 flex flex-col gap-3 sm:pl-4 overflow-y-auto pr-2 max-h-[180px] sm:max-h-[250px] w-full">
          {chartData.map((entry, index) => (
            <div key={entry.name} className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-3 shrink-0">
                <span className="font-mono text-muted-foreground">{entry.percentage}%</span>
                <span className="font-mono font-medium">{entry.views.toLocaleString()}</span>
              </div>
            </div>
          ))}
        </div>
