...
export default function VideoSeoClient({ initialData }: { initialData?: any }) {
  const [videos, setVideos] = useState<Video[]>([]);
  const [loading, setLoading] = useState(true);
  const [, startTransition] = useTransition();

  const parsedInput = initialData && (typeof initialData.input === 'string' ? JSON.parse(initialData.input) : initialData.input);
  const initialPrompt = parsedInput?.prompt || parsedInput?.content || "";
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-[calc(100vh-4rem)]">
      <div className="w-full max-w-4xl mx-auto py-8 space-y-8">
        ...
        {/* Search bar */}
        <VideoSeoSearchBar onGenerated={handleGenerated} initialPrompt={initialPrompt} initialOptions={parsedInput?.options} />
